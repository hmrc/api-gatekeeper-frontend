...
trait ApplicationResponseMock extends ApplicationResponseBuilder with CollaboratorsMock with AccessMock with ApplicationStateMock {
  ...
  val blockedApplicationResponse = applicationResponseTest
    .withId(ApplicationId(newBlockedApplicationWithSubscriptionDataId))
    .withName(newBlockedApplicationName)
    .withBlocked(true)

  val pendingApprovalApplicationResponse = applicationResponseTest
