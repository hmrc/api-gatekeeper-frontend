...
@Singleton
class ApiPlatformAdminApiTestController @Inject() (
    mcc: MessagesControllerComponents,
    connector: ApiPlatformAdminApiConnector,
  ...
  def page(applicationId: ApplicationId): Action[AnyContent] = anyAuthenticatedUserAction { implicit request =>
    for {
      application <- connector.getApplication(applicationId, hc)
    } yield Ok(application.fold("Application not found")(app => app.asJson.toString()))
  }
}
