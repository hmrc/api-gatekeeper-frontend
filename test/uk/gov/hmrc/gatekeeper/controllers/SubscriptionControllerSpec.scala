  ...
  running(app) {
    ...
    "subscribeToApi" should {
      ...
      "call the service to subscribe to the API when submitted for a super user" in new Setup {
        val subscribeToApi = SubscribeToApi(Actors.GatekeeperUser(userToken._2), apiIdentifier, LocalDateTime.now(fixedClock))
    ...
    "unsubscribeFromApi" should {
      ...
      "call the service to unsubscribe from the API when submitted for a super user" in new Setup {
        val unsubscribeFromApi = UnsubscribeFromApi(Actors.GatekeeperUser(userToken._2), apiIdentifier, LocalDateTime.now(fixedClock))
        StrideAuthorisationServiceMock.Auth.succeeds(GatekeeperRoles.SUPERUSER)
        givenTheAppWillBeReturned()
