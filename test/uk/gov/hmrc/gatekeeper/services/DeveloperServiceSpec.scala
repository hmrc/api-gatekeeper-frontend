...
class DeveloperServiceSpec extends AsyncHmrcSpec with CollaboratorTracker {
  ...
  trait Setup extends MockitoSugar with ArgumentMatchersSugar with ApplicationConnectorMockProvider
    ...
    def fetchDeveloperWillReturn(user: RegisteredUser, productionApps: List[ApplicationResponse] = List.empty,
  ...
  "developerService searchDevelopers" should {
    "find users" in new Setup {
      ...
      val filter = DevelopersSeachFilter(maybeEmailFilter = Some(emailFilter))
    ...
    "find by api context and version" in new Setup {
      ...
      val filter = DevelopersSeachFilter(environmentFilter = AnyEnvironment, maybeApiFilter = Some(ApiContextVersion(apiContext, apiVersion)))
    ...
    "find by api context and version where same email in production and sandbox" in new Setup {
      ...
      val filter = DevelopersSeachFilter(maybeApiFilter = Some(ApiContextVersion(apiContext, apiVersion)))
    ...
    "find by api context and version with email filter with a collaborator who is not registered" in new Setup {
      ...
      val filter = DevelopersSeachFilter(maybeEmailFilter = Some(emailFilter), maybeApiFilter = Some(ApiContextVersion(apiContext, apiVersion)))
    ...
    "find by developer status" in new Setup {
      ...
      val filter = DevelopersSeachFilter(None, None, developerStatusFilter = DeveloperStatusFilter.VerifiedStatus)
      val result = await(underTest.searchDevelopers(filter))
    ...
    "find by api context, version and developer status" in new Setup {
      ...
      val filter = DevelopersSeachFilter(maybeEmailFilter =
    ...
    "find by developer status should sort users by email" in new Setup {
      ...
      val filter = DevelopersSeachFilter(None, None, developerStatusFilter = DeveloperStatusFilter.AllStatus)
      val result = await(underTest.searchDevelopers(filter))
    ...
    "find by api context and version and Production environment" in new Setup {
      ...
      val filter = DevelopersSeachFilter(environmentFilter = ProductionEnvironment, maybeApiFilter = Some(ApiContextVersion(apiContext, apiVersion)))
    ...
    "find by api context and version and Sandbox environment" in new Setup {
      ...
      val filter = DevelopersSeachFilter(environmentFilter = SandboxEnvironment, maybeApiFilter = Some(ApiContextVersion(apiContext, apiVersion)))
