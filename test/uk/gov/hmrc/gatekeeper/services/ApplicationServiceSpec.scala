...
class ApplicationServiceSpec extends AsyncHmrcSpec with ResetMocksAfterEachTest {
  ...
  "fetchProdAppStateHistories" should {
    ...
    "handle apps with single state history item correctly" in new Setup {
      ...
      result shouldEqual List(
        ApplicationStateHistoryChange(appStateHistory.applicationId.value, appStateHistory.name, appStateHistory.journeyVersion.toString, "TESTING", appStateHistory.stateHistory(0).timestamp.toString, "", "")
      )
    }
    ...
    "handle apps with multiple state history items correctly" in new Setup {
      ...
      result shouldEqual List(
        ApplicationStateHistoryChange(appStateHistory.applicationId.value, appStateHistory.name, appStateHistory.journeyVersion.toString, "TESTING", appStateHistory.stateHistory(0).timestamp.toString, "PENDING_GATEKEEPER_APPROVAL", appStateHistory.stateHistory(1).timestamp.toString),
        ApplicationStateHistoryChange(appStateHistory.applicationId.value, appStateHistory.name, appStateHistory.journeyVersion.toString, "PENDING_GATEKEEPER_APPROVAL", appStateHistory.stateHistory(1).timestamp.toString, "PRODUCTION", appStateHistory.stateHistory(2).timestamp.toString),
        ApplicationStateHistoryChange(appStateHistory.applicationId.value, appStateHistory.name, appStateHistory.journeyVersion.toString, "PRODUCTION", appStateHistory.stateHistory(2).timestamp.toString, "", "")
      )
    }
    "handle multiple apps in response correctly" in new Setup {
      ...
      result shouldEqual List(
        ApplicationStateHistoryChange(app1StateHistory.applicationId.value, app1StateHistory.name, app1StateHistory.journeyVersion.toString, "TESTING", app1StateHistory.stateHistory(0).timestamp.toString, "", ""),
        ApplicationStateHistoryChange(app2StateHistory.applicationId.value, app2StateHistory.name, app2StateHistory.journeyVersion.toString, "TESTING", app2StateHistory.stateHistory(0).timestamp.toString, "PRODUCTION", app2StateHistory.stateHistory(1).timestamp.toString),
        ApplicationStateHistoryChange(app2StateHistory.applicationId.value, app2StateHistory.name, app2StateHistory.journeyVersion.toString, "PRODUCTION", app2StateHistory.stateHistory(1).timestamp.toString, "", ""),
      )
    }
  }
