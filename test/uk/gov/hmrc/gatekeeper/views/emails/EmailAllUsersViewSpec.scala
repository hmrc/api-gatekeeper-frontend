  ...
  "email all user view" must {
    ...
    "show correct title and content for 2 verified users" in new Setup {
      ...
      val result: HtmlFormat.Appendable =
        emailAllUsersView.render(users, emailRecipientsAsJson, s"${user1.email.text}; ${user2.email.text}", request, LoggedInUser("Bobby Example"), messagesProvider)

      validateEmailAllUsersPage(Jsoup.parse(result.body), users)
    }

    "show correct title and content for empty / no users" in new Setup {
      val result: HtmlFormat.Appendable = emailAllUsersView.render(Seq.empty, emailRecipientsAsJson, "", request, LoggedInUser("Bobby Example"), messagesProvider)
