...
import uk.gov.hmrc.gatekeeper.views.html.emails.{EmailPreferencesSelectTopicView, EmailPreferencesSelectUserTopicView}
...
class EmailPreferencesSelectUserTopicViewSpec extends CommonViewSpec with EmailPreferencesTopicViewHelper {
  ...
  "email preferences select user topic view" must {

    "show correct title and options while selecting user topic" in new Setup {
      val result: HtmlFormat.Appendable =
        emailPreferencesSelectUserTopicView.render(Some(TopicOptionChoice.TECHNICAL), request, LoggedInUser("Bobby Example"), messagesProvider)
