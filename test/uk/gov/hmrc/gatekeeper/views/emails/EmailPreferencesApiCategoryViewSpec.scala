  ...
  "email preferences category view" should {
    ...
    "show correct title and options when no filter provided and empty list of users" in new Setup {
      val result: HtmlFormat.Appendable =
        emailPreferencesApiCategoryView.render(Seq.empty, emailRecipientsAsJson, "", Some(BUSINESS_AND_POLICY), categories, "", "", request, LoggedInUser("Bobby Example"), messagesProvider)
    ...
    "show correct title and options when only Category filter provided" in new Setup {
      // If adding errors to the page we need to add tests in here for that message
      val result: HtmlFormat.Appendable =
        emailPreferencesApiCategoryView.render(Seq.empty, emailRecipientsAsJson, "", None, categories, category1.category, "", request, LoggedInUser("Bobby Example"), messagesProvider)
