...
class ApplicationServiceSpec extends UnitSpec with MockitoSugar {
  ...
  trait Setup {
    ...
    val stdApp1 = ApplicationResponse(UUID.randomUUID(), "application1", "clientid1", "PRODUCTION", None, collaborators, DateTime.now(), Standard(), ApplicationState())
    val stdApp2 = ApplicationResponse(UUID.randomUUID(), "application2", "clientid2", "PRODUCTION", None, collaborators, DateTime.now(), Standard(), ApplicationState())
    val privilegedApp = ApplicationResponse(UUID.randomUUID(), "application3", "clientid3", "PRODUCTION", None, collaborators, DateTime.now(), Privileged(), ApplicationState())
    val ropcApp = ApplicationResponse(UUID.randomUUID(), "application4", "clientid4", "PRODUCTION", None, collaborators, DateTime.now(), Ropc(), ApplicationState())
